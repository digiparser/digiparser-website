export const RenderBlocks = ({ blocks }) => {
    ...
    return groupedBlocks.map((block) => {
        if (Array.isArray(block.items)) {
            const ListComponent = block.type === 'numbered_list_item' ? OrderedList : UnorderedList
            return (
                <ListComponent key={block.items[0].id}>
                    {block.items.map((item) => (
                        <ListItem key={item.id} value={item[item.type]} id={item.id} />
                    ))}
                </ListComponent>
            )
        }
...
const ListItem = ({ value, id }) => {
    return <SpanText text={value.rich_text} id={id} />
}
